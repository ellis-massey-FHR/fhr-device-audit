...
def get_current_workstation_from_ci(user_sys_id):
    ...
    for r in records:
        ...
        if isinstance(model_id, dict):
            model_name = model_id.get("display_value", "").lower()
        ...
        trust_raw = r.get("u_device_trust")
        device_trust = trust_raw.lower() if isinstance(trust_raw, str) else ""

        # ✅ Only allow Windows devices with AzureAD or HybridAD trust
        if "windows" not in os_name or device_trust not in ("azuread", "hybridad"):
            print(f"🛑 Skipping device: {name} (OS: {os_name}, Trust: {device_trust})")
            continue
...
#--------------------------Write to CSV and Excel-------------------------
    ...
    print("📁 Data written to filtered_laptop_requests.csv")
